...
export class RestauranteService {
    ...
    async findAll(): Promise<RestauranteEntity[]> {
       ...
       if(!cached){
           const restaurantes: RestauranteEntity[] = await this.restauranteRepository.find({ relations: ["estrellasMichellin", "culturas"]});
    ...
    async findOne(id: string): Promise<RestauranteEntity> {
        const restaurante: RestauranteEntity = await this.restauranteRepository.findOne({where: {id}, relations: ["estrellasMichellin", "culturas"] } );
        if (!restaurante)
